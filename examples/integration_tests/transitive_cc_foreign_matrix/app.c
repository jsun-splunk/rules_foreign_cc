...
static const void *find_loaded_symbol(const char *const *module_names, const char *symbol_name) {
...
static void print_loaded_symbol_path(
    const char *name,
...
static void print_optional_loaded_symbol_path(const char *name, const char *symbol_name) {
#ifdef _WIN32
    const void *symbol;
...
static size_t write_archive(char *buffer, size_t buffer_size) {
    ...
    if (archive_write_open_memory(archive, buffer, buffer_size, &used) != ARCHIVE_OK) {
...
int main(void) {
    ...
    puts(kExpected);
    print_loaded_symbol_path(
        "libarchive",
        (const void *)&archive_read_new,
        "archive_read_new"
#ifdef _WIN32
        ,
