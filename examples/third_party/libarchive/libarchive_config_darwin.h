/* config.h.  Best-effort Darwin config for libarchive 3.7.7 native cc_library examples. */
...
/*
 * This header starts from the Linux CMake-generated config used by the native
 * cc_library example, then overrides platform-sensitive probes for Darwin. It
 * intentionally keeps ACL and xattr disabled, matching the example CMake
 * configuration and keeping the native test focused on libarchive+zlib linking.
 */
#if !defined(__APPLE__)
#error libarchive_config_darwin.h is only intended for Darwin builds.
#endif
