...
def get_flags_info(ctx, link_output_file = None, runtime_search_context = None):
    ...
    Args:
        ...
        link_output_file: output file to be specified in the link command line
            flags
        runtime_search_context: install-root and declared-output
            metadata used to derive runtime library search directories
    ...
    if targets_windows(ctx, cc_toolchain_):
        ...
        runtime_search_directories = runtime_library_search_directories(ctx, runtime_search_context)
